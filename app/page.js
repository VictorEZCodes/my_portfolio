...
function Waveform() {
  ...
  // Generate bar heights — irregular pattern that looks like a real waveform
  ...
  return (
    <div className="relative w-full max-w-xs sm:max-w-sm h-48 sm:h-64 flex items-center justify-center">
      {/* Glow behind waveform when playing */}
          ...
          return (
            <div
              ...
              className={`rounded-full transition-all ${
                playing
                  ? 'bg-glow waveform-bar-active'
                  : 'bg-warm-300/40 dark:bg-warm-700/30'
              }`}
              style={{
                width: barWidth,
      ...
      <div className={`absolute -bottom-2 left-1/2 -translate-x-1/2 text-[10px] tracking-widest transition-opacity duration-500 ${
        playing ? 'text-glow opacity-100' : 'text-warm-400 dark:text-warm-600 opacity-60'
      }`}>
...
function TrackRow({ number, project, onHover, onLeave }) {
  ...
  return (
    <Link href={project.link || '#'} target="_blank" rel="noopener noreferrer">
      ...
      >
        <span className="w-8 text-right text-sm tabular-nums text-warm-400 dark:text-warm-600 group-hover:text-glow transition-colors">
        ...
        <span className="text-xs text-warm-400 dark:text-warm-600 tabular-nums hidden sm:block">
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="min-h-screen flex items-center relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 w-full py-32">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            >
              <p className="text-sm text-warm-500 dark:text-warm-500 mb-4 tracking-wide">
              ...
              <h1 className="text-6xl sm:text-7xl lg:text-8xl font-bold leading-[0.9] mb-6">
                ...
                <span className="block text-glow">Ezeanyika</span>
              </h1>
              <p className="text-warm-600 dark:text-warm-400 max-w-md text-base leading-relaxed mb-8">
                ...
                {socials.map((social, i) => {
                  ...
                  return (
                    <motion.a
                      ...
                      className="text-warm-400 dark:text-warm-600 hover:text-glow transition-colors duration-200"
                      whileHover={{ y: -2 }}
        ...
        >
          <span className="text-xs text-warm-400 dark:text-warm-600 tracking-widest">SCROLL</span>
          <div className="w-px h-8 bg-gradient-to-b from-warm-400 dark:from-warm-600 to-transparent" />
        </motion.div>
      </section>

      {/* === STATS STRIP === */}
      <section className="border-y border-warm-200/50 dark:border-warm-800/50">
          ...
          >
            {[
              ...
              <div key={i} className="text-center">
                <p className="text-2xl sm:text-3xl font-bold text-glow">{stat.value}</p>
                <p className="text-xs text-warm-500 dark:text-warm-500 tracking-wide mt-0.5">{stat.label}</p>
              </div>
            ))}
          </motion.div>
        </div>
        ...
        <div className="max-w-3xl mx-auto px-6">
          ...
          >
            <div className="flex items-center justify-between mb-8">
              <div>
                <p className="text-xs text-warm-500 dark:text-warm-500 tracking-widest mb-1">TRACKLIST</p>
                <h2 className="text-2xl font-bold">Featured Projects</h2>
              </div>
              <Link
                href="/projects"
                className="text-sm text-warm-500 hover:text-glow transition-colors"
              >
                View all {visibleProjects.length} →
              </Link>
            </div>

            <div className="h-px bg-warm-200 dark:bg-warm-800 mb-2" />
            ...
            <div className="h-px bg-warm-200 dark:bg-warm-800 mt-2" />
          </motion.div>
        </div>
      </section>

      {/* === EXPERIENCE === */}
      <section className="py-24 bg-warm-100/50 dark:bg-warm-900/30">
        <div className="max-w-3xl mx-auto px-6">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="flex items-center justify-between mb-12">
              <div>
                <p className="text-xs text-warm-500 dark:text-warm-500 tracking-widest mb-1">LINER NOTES</p>
              ...
              {experiences.map((exp, i) => (
                <div key={exp.id} className="flex gap-6 group">
                  <div className="flex flex-col items-center">
                    <div className="w-2.5 h-2.5 rounded-full bg-glow mt-1.5 flex-shrink-0" />
                  ...
                  <div className="pb-10">
                    <div className="flex items-center gap-3 mb-1">
                      ...
                      <h3 className="font-semibold text-base">
                        {exp.title}
                        <span className="text-warm-500 dark:text-warm-500 font-normal"> at </span>
                    ...
                    <p className="text-xs text-warm-500 dark:text-warm-500 mb-2">
                      {exp.duration} <span className="text-glow font-semibold">· {formatDuration(exp.duration)}</span> — {exp.location}
                    </p>
                    <p className="text-sm text-warm-600 dark:text-warm-400 leading-relaxed mb-3">
                      {exp.description[0]}
                    </p>
      ...
      {/* === FOOTER === */}
      <footer className="py-16 border-t border-warm-200/50 dark:border-warm-800/50">
        <div className="max-w-3xl mx-auto px-6 text-center">
          <p className="text-sm text-warm-500 dark:text-warm-500 mb-4">Get in touch</p>
          ...
          <p className="text-xs text-warm-400 dark:text-warm-700">
